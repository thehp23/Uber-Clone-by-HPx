...
captainSchema.methods.generateAuthToken = function(){ //jwt Authentication
    const token = jwt.sign({_id : this._id},process.env.JWT_SECRET,{expiresIn : '24h'});
    return token; //token generated
}

captainSchema.methods.comparePassword = async function(password){ //compare password with the help of bcrypt
    return await bcrypt.compare(password,this.password)
}

captainSchema.statics.hashPassword = async (password) =>{ //hash password
